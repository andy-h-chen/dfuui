...
utils = {
    ...
    connectToDatabase: function(mongoose, config, cb) {
        return mongoose.connect(utils.getDbPath(config), cb);
    },
    ...
    parseDbErrors: function(err, errorMessages, uniqueField) {
        var response = {},
            errors = {};
        ...
        if (err.name && err.name === 'MongoError') {
            ...
            if (err.code === 11000 || err.code === 11001) {
                ...
                ret.errors[uniqueField] = errorMessages.DUPLICATE;
                return ret;
            } else {
                return {
                    code: 500,
    ...
    getRandDate: function(from, date, unit) {
        ...
        if (!from) {
            ...
            if (!date) {
                throw new Error('Please provide a date for getRandDate(from, date)');
            }
    ...
    sendEmailFromAuto: function(config, mailOptions, callback) {
		var cfg = {
            host: config.SMTP,
            port: config.SMTP_PORT,
			secureConnection: config.SMTP_SECURE == true
        };
        ...
        var transporter = nodemailer.createTransport(cfg);
        // send mail with defined transport object
        transporter.sendMail(mailOptions, function(error, info){
            if(error) {
                console.log('Error on sendEmailFromAuto:', error);
            }else{
                console.log('sendEmailFromAuto ' + info.response);
            }
            if (callback) {
                callback(error, info);
            }
