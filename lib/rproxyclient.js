  ...
  this.processResponse = function() {
  	if (self.responseQueue.length > 0) {
        ...
        console.log('msg.length = ', msg.length, ' msg.imgLen = ', msg.imgLen, ' msg.currentLength = ', msg.currentLength);
  		...
  		var xmlBuffer = new Buffer.allocUnsafe(msg.length);
        ...
        if (result && result.Interaction
          ...
          var imgData;
          if (msg.imgLen > 0) {
            var imgBuffer = new Buffer.allocUnsafe(msg.imgLen);
  ...
  this.dataHandler = function(data) {
    ...
    while (dataPosition != data.length) {
    	...
    	if (!header) {
    		...
    		var msg = self.responseQueue[self.responseQueue.length - 1];
    		...
    		msg.dataBuffer = new Buffer.allocUnsafe(header.length);
    		...
    		self.responseQueue.push(msg);
    	}
  		logger.debug('RproxyClient.dataHandler dataPosition = ' + dataPosition + ' queue.size = ' + self.responseQueue.length);
    }
    setTimeout(self.processResponse, 0);
  };
...
function RproxyClientAuth(app, config, mongoose, logger) {
  ...
  this.dataHandler = function(data) {
    ...
    var dataBuffer = new Buffer.allocUnsafe(header.length);
    ...
    logger.info("RproxyClientAuth.dataHandler", xmlStr);
  ...
  var handleMessageWithImage = function(result, data, callback) {
    ...
    if ((data.length > self.currentHeader.length + rproxyClientUtils.TlvHeaderLength) && dataLength != 0) {
      ...
      self.currentResult = result;
      self.currentImageBuffer = new Buffer.allocUnsafe(dataLength);
      self.currentImageBufferContentLength = dataLength;
      self.readImageCallback = callback;
      // more data to come
