...
var RproxyDataHandler = function(data) {
  if (!data) return;
  //console.log('RproxyDataHandler data.length = ' + data.length);
  ...
  while (dataPosition != data.length) {
    ...
    //console.log("RproxyDataHandler header = ", header);
    if (!header) {
      ...
      if (msgLen > msg.currentLength) {
        var lengthToCopy = Math.min(msgLen, data.length);
        data.copy(msg.dataBuffer, msg.currentLength, dataPosition, dataPosition + lengthToCopy);
        dataPosition += lengthToCopy;
        msg.currentLength += lengthToCopy;
        //console.log('lengthToCopy=', lengthToCopy);
        //console.log(msg);
      } else {
      ...
      msg.dataBuffer = new Buffer.alloc(header.length + header.imgLen, 0);
  ...
  this.processResponse = function() {
    ...
    if (msg.length + msg.imgLen != msg.currentLength) {
      if (self.responseQueue.length == 1) {
        // msg not complete, wait for more data
        logger.info('RproxyClientAuth processResponse Msg not complete, wait for more data. msg.length = ',
                     msg.length, ' msg.imgLen = ', msg.imgLen, ' msg.currentLength = ', msg.currentLength);
        return;
      }
    ...
    var xmlBuffer = Buffer.alloc(msg.length, 0);
    //console.log("msg.dataBuffer.length=", msg.dataBuffer.length);
    msg.dataBuffer.copy(xmlBuffer, 0, 0, msg.length);
    var xmlStr = xmlBuffer.toString('utf8');
    xml2js.parseString(xmlStr, function(err, result) {
      ...
      if (result && result.Interaction) {
        ...
        var imgBuffer;
        if (msg.imgLen > 0) {
          imgBuffer = Buffer.alloc(msg.imgLen, 0);
          msg.dataBuffer.copy(imgBuffer, 0, msg.length);
        }
    ...
    var registerOrCheckUser = function(dfu) {
      User.findByUsername(email, function(err, user) {
        ...
        if (user) {
          ...
          var hash = require('password-hasher').createHash('ssha512', tempPassword, new Buffer.from(app.salt, 'hex')),
    ...
    sendEmailForNewUser: function(config, user, dfu, lng) {
      changeLanguage(checkLng(lng), function() {
        var mailOptions = {
          ...
          html: i18next.t('email.newUser.html',
            {
              deviceName: dfu.deviceName,
              loginLink: config.URL + config.V1_LOGIN + '?username=' + user.username + '&password=' + user.plainPassword,
              email: user.email,
